...
@section('title')
  Смотри онлайн на Результаты поиска | 2kino
@endsection
@section('content')
  <div class="content_2">
    <div class="heading"><h1>Результаты поиска:</h1></div>
    <div class="grid">
        @if (count($film))
          @foreach ($film as $f)
          ...
          @endforeach
          @else
            <p>Ой =)! Ничего не нашлось</p>
        @endif
    </div>
    <div class="pagination">{{ $film->appends(['s' => request()->s])->links() }}</div>
  </div>
@endsection
